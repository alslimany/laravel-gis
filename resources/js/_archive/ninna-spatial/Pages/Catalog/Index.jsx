...
import { Heading, PageHeader, Panel, PrimaryButton, TextInput } from '../../Components/ui';

function ResultList({ title, items, hrefFor }) {
    return (
        <Panel>
            <Heading as="h2" className="border-b border-line">
                {title} ({items.length})
            </Heading>
            <ul>
                {items.length === 0 ? <li className="px-3 py-3 text-muted">No matches.</li> : null}
                {items.map((item) => (
                    <li key={item.id} className="flex items-center justify-between border-b border-line/70">
                        <span>
                            {item.name}
                            <span className="mt-0.5 block text-muted">{item.geometry_type || item.description || ''}</span>
                        </span>
                        {hrefFor ? (
