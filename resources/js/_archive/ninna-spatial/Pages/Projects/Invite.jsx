...
import { DangerButton, Field, GhostLink, PageHeader, PrimaryButton, Select, Table, DataTable, thClass, tdClass, tdMuted, tdMono } from '../../Components/ui';
    ...
    return (
        <AppLayout title={`Invite · ${project.name}`}>
            ...
            {availableUsers.length ? (
                ...
                <p className="mb-4 text-muted">No other users in this organization are available to invite.</p>
            )}
            <DataTable>
                <Table.Header>
                    <Table.Row>
                    ...
                    {collaborators.map((person) => (
                        <Table.Row key={person.id}>
                            <Table.Cell className={tdClass}>
                                {person.name}
                                <div className="text-[12px] text-base-600">{person.email}</div>
                            </Table.Cell>
                            <Table.Cell className={tdClass}>
                                <Select
