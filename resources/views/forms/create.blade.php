    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('forms.store') }}" id="formBuilder">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('layer_id') is-invalid @enderror" id="layer_id" name="layer_id" required>
                                ...
                                @foreach($layers as $layer)
                                    <option value="{{ $layer->id }}" @selected(old('layer_id') == $layer->id)>
                                        {{ $layer->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="is_public" name="is_public" value="1" @checked(old('is_public'))>
                        ...
                        <div id="schemaFields">
                            @php $oldSchema = old('schema', [['name' => '', 'label' => '', 'type' => 'text', 'required' => false]]); @endphp
                            @foreach($oldSchema as $i => $field)
                                <div class="border rounded p-3 mb-2 schema-row">
                                    <div class="row g-2">
                                        <div class="col-md-3">
                                            <label class="form-label">Field name</label>
                                            <input type="text" class="form-control" name="schema[{{ $i }}][name]" value="{{ $field['name'] ?? '' }}" placeholder="column_name">
                                        </div>
                                        <div class="col-md-3">
                                        ...
                                        <div class="col-md-1 d-flex align-items-end">
                                            <button type="button" class="btn btn-outline-danger btn-sm remove-field mb-2" title="Remove">&times;</button>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
